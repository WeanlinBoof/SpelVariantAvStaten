	...
	public static class FSCollisions {
		...
		public static bool TestOverlap(Shape shapeA, Shape shapeB, ref FSTransform transformA,
			...
			if (shapeA.ChildCount > 1) {
				for (int i = 0; i < shapeA.ChildCount; i++) {
			...
			if (shapeB.ChildCount > 1) {
				for (int i = 0; i < shapeB.ChildCount; i++) {
		...
		private static bool CollidePolygons(PolygonShape polygonA, ref FSTransform transformA, PolygonShape polygonB,
									ref FSTransform transformB, out FSCollisionResult result) {
			result = new FSCollisionResult();
			...
			if (_manifold.PointCount > 0) {
				...
				if (_manifold.Type == ManifoldType.FaceA) {
					...
					Vector2 planePoint = MathUtils.Mul(ref transformA, _manifold.LocalPoint);

					Vector2 clipPoint = MathUtils.Mul(ref transformB, _manifold.Points[0].LocalPoint);
					float separation = Vector2.Dot(clipPoint - planePoint, result.Normal) - polygonA.Radius -
									 polygonB.Radius;
					...
					Vector2 planePoint = MathUtils.Mul(ref transformB, _manifold.LocalPoint);

					Vector2 clipPoint = MathUtils.Mul(ref transformA, _manifold.Points[0].LocalPoint);
					float separation = Vector2.Dot(clipPoint - planePoint, result.Normal) - polygonA.Radius -
									 polygonB.Radius;
					result.Point = clipPoint * FSConvert.SimToDisplay;
		...
		private static bool CollidePolygonCircle(PolygonShape polygon, ref FSTransform polyTransform, CircleShape circle,
			...
			if (_manifold.PointCount > 0) {
				...
				Vector2 circleInPolySpace = MathUtils.MulT(ref polyTransform, circleTransform.P);
				Vector2 value1 = circleInPolySpace - _manifold.LocalPoint;
				Vector2 value2 = _manifold.LocalNormal;
				float separation = circle.Radius - (value1.X * value2.X + value1.Y * value2.Y);
		...
		private static bool CollideCircles(CircleShape circleA, ref FSTransform firstTransform, CircleShape circleB,
			...
			if (_manifold.PointCount > 0) {
				...
				Vector2 pointA = MathUtils.Mul(ref firstTransform, _manifold.LocalPoint);
				Vector2 pointB = MathUtils.Mul(ref secondTransform, _manifold.Points[0].LocalPoint);

				result.Normal = pointA - pointB;
				Vector2Ext.Normalize(ref result.Normal);

				Vector2 cA = pointA - circleA.Radius * result.Normal;
				Vector2 cB = pointB + circleB.Radius * result.Normal;
				result.Point = 0.5f * (cA + cB);
				result.Point *= FSConvert.SimToDisplay;

				float separation = Vector2.Dot(pointA - pointB, result.Normal) - circleA.Radius - circleB.Radius;
		...
		private static bool CollideEdgeAndCircle(EdgeShape edge, ref FSTransform edgeTransform, CircleShape circle,
										 ref FSTransform circleTransform, out FSCollisionResult result) {
			result = new FSCollisionResult();
			...
			if (_manifold.PointCount > 0) {
				...
				if (_manifold.Type == ManifoldType.Circles) {
					// this is essentically directly from ContactSolver.WorldManifold.Initialize. To avoid doing the extra math twice we duplicate this code
					// here because it doesnt return some values we need to calculate separation
					Vector2 pointA = MathUtils.Mul(ref edgeTransform, _manifold.LocalPoint);
					Vector2 pointB = MathUtils.Mul(ref circleTransform, _manifold.Points[0].LocalPoint);

					result.Normal = pointA - pointB;
					Vector2Ext.Normalize(ref result.Normal);

					Vector2 cA = pointA - edge.Radius * result.Normal;
					Vector2 cB = pointB + circle.Radius * result.Normal;
					result.Point = 0.5f * (cA + cB);
					result.Point *= FSConvert.SimToDisplay;

					float separation = Vector2.Dot(pointA - pointB, result.Normal) - edge.Radius - circle.Radius;
				...
				else // FaceA
				{
					...
					Vector2 clipPoint = MathUtils.Mul(ref circleTransform, _manifold.Points[0].LocalPoint);
					float separation = Vector2.Dot(clipPoint - planePoint, result.Normal) - edge.Radius - circle.Radius;
