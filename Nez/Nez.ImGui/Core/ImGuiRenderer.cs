...
namespace Nez.ImGuiTools {
	...
	public class ImGuiRenderer {
		...
		private readonly int _vertexDeclarationSize;
		...
		public unsafe void RebuildFontAtlas(ImGuiOptions options) {
			...
			foreach (Tuple<string, float> font in options._fonts) {
			...
			Texture2D tex2d = new Texture2D(Core.GraphicsDevice, width, height, false, SurfaceFormat.Color);
 ...
 Marshal.GetFunctionPointerForDelegate<GetClipboardTextDelegate>( SDL2.SDL.SDL_GetClipboardText );
		...
		private void RenderDrawData(ImDrawDataPtr drawData) {
			...
			Viewport lastViewport = Core.GraphicsDevice.Viewport;
			Rectangle lastScissorBox = Core.GraphicsDevice.ScissorRectangle;
