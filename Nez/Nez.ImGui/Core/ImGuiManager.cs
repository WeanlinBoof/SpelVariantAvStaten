...
namespace Nez.ImGuiTools {
	public partial class ImGuiManager : GlobalManager, IFinalRenderDelegate, IDisposable {
		...
		private List<Type> _sceneSubclasses = new List<Type>();
		private System.Reflection.MethodInfo[] _themes;
		private CoreWindow _coreWindow = new CoreWindow();
		private SceneGraphWindow _sceneGraphWindow = new SceneGraphWindow();
		private SpriteAtlasEditorWindow _spriteAtlasEditorWindow;
		private List<EntityInspector> _entityInspectors = new List<EntityInspector>();
		private List<Action> _drawCommands = new List<Action>();
		...
		private void DrawMainMenuBar() {
			if (ImGui.BeginMainMenuBar()) {
				...
				if (_sceneSubclasses.Count > 0 && ImGui.BeginMenu("Scenes")) {
					foreach (Type sceneType in _sceneSubclasses) {
				...
				if (_themes.Length > 0 && ImGui.BeginMenu("Themes")) {
					foreach (System.Reflection.MethodInfo theme in _themes) {
				...
				if (ImGui.BeginMenu("Game View")) {
					Point rtSize = Core.Scene.SceneRenderTargetSize;
					...
					if (ImGui.BeginMenu("Reposition")) {
						foreach (string pos in Enum.GetNames(typeof(WindowPosition))) {
		...
		private void DrawEntityInspectors() {
			for (int i = _entityInspectors.Count - 1; i >= 0; i--) {
		...
		public void StartInspectingEntity(Entity entity) {
			...
			foreach (EntityInspector inspector in _entityInspectors) {
