...
namespace Nez.ImGuiTools.TypeInspectors {
	...
	public class SimpleTypeInspector : AbstractTypeInspector {
		...
		private Action _inspectMethodAction;
		private bool _isUnsignedInt;
		...
		public override void Initialize() {
			...
			string valueTypeName = _valueType.Name.ToString();
			string inspectorMethodName = "Inspect" + valueTypeName[0].ToString().ToUpper() + valueTypeName.Substring(1);
			System.Reflection.MethodInfo inspectMethodInfo = ReflectionUtils.GetMethodInfo(this, inspectorMethodName);
		...
		private void InspectColor() {
			Num.Vector4 value = GetValue<Color>().ToNumerics();
		...
		/// <returns></returns>
		private bool InspectAnyInt(ref int value) {
		...
		private void InspectUInt32() {
			int value = Convert.ToInt32(GetValue());
			if (InspectAnyInt(ref value)) {
				SetValue(Convert.ToUInt32(value));
			}
		}

		private void InspectInt64() {
			int value = Convert.ToInt32(GetValue());
			if (InspectAnyInt(ref value)) {
		...
		private unsafe void InspectUInt64() {
			int value = Convert.ToInt32(GetValue());
			if (InspectAnyInt(ref value)) {
		...
		private void InspectString() {
			string value = GetValue<string>() ?? string.Empty;
