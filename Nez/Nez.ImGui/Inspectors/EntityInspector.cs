...
namespace Nez.ImGuiTools {
	public class EntityInspector {
		...
		private List<IComponentInspector> _componentInspectors = new List<IComponentInspector>();
		...
		public void Draw() {
			...
			if (ImGui.Begin($"Entity Inspector: {Entity.Name}###" + _entityWindowId, ref open)) {
				...
				for (int i = _componentInspectors.Count - 1; i >= 0; i--) {
		...
		private void DrawComponentSelectorPopup() {
			if (ImGui.BeginPopup("component-selector")) {
				...
				bool isNezType = false;
				bool isColliderType = false;
				foreach (Type subclassType in InspectorCache.GetAllComponentSubclassTypes()) {
