	...
	public sealed class NsonEncoder : INsonEncoder {
		...
		public void EncodeValue(object value) {
			if (value == null) {
				...
				string output = ((DateTime)value).ToString(NsonConstants.iso8601Format[0], CultureInfo.InvariantCulture);
				EncodeString(output);
			}
		...
		public void EncodeObject(object value) {
			...
			NsonTypeConverter converter = _settings.GetTypeConverterForType(type);
		...
		private void EncodeArrayRank(Array value, int rank, int[] indices) {
			...
			if (rank == value.Rank - 1) {
				Type arrayItemType = value.GetType().GetElementType();
